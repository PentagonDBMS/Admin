...
const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const checkAdminLoggedIn = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}/api/admin/auth/user`, {
                    method: 'GET',
                ...
                console.error("Failed to check if admin is logged in:", error);
                setCurrentUser(null);
    ...
    const register = async (name, email, password) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/admin/auth/signup`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
